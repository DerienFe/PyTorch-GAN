...
"""
transform = transforms.Compose(
        [transforms.RandomResizedCrop(opt.img_size, scale=(0.95, 1.0)),
         transforms.RandomHorizontalFlip(),
         transforms.ToTensor(),
         #transforms.Normalize(mean=[0.485, 0.456, 0.406],std=[0.229, 0.224, 0.225]),
         ])

transformed_self_dataset = Self_dataset(PATH = './cat_images/', transform = transform)
#dataloader = torch.utils.data.DataLoader(transformed_self_dataset,batch_size=opt.batch_size, shuffle=True, num_workers=4)
dataloader = torch.utils.data.DataLoader(transformed_self_dataset,batch_size=opt.batch_size, shuffle=True)
...
for epoch in range(opt.n_epochs):
    for i, imgs in enumerate(dataloader):
        ...
        #total_time = (opt.n_epochs+1)*(len(dataloader)+1)*(time.time() - start)
        #remaining_time = total_time - (epoch+1)*(i+1)*(time.time() - start)
        print(
            "[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [time used(sec): %f]]"
            % (epoch, opt.n_epochs, i, len(dataloader), d_loss.item(), g_loss.item(), (time.time() - start))
        )
